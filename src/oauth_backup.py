...
class OAuthClient(object):
    ...
    def login(self):
        session_id = self.get_cookie('oauth.twitter')
        if session_id:
            self.expire_cookie()
    ...
    def get_request_token(self):
        ...
        req_token = OAuthRequestToken(service=self.service, **params)
        req_token.put()
    ...
    def callback(self, return_to='/'):
        ...
        req_token_query = OAuthRequestToken.all() \
                                           .filter('oauth_token =', 
                                                   oauth_token) \
                                           .filter('service =', self.service)

        if req_token_query.count() > 0:
            oauth_token = req_token_query.fetch(1)[0]
        else:
            return self.get_request_token()
